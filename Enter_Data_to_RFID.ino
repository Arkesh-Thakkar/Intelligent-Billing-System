...
<h2>Intelligent Billing System<h2>
<h2>By A2V2<h2>
<h3> Product Form </h3>
...
//Sector 1 block 5 data in 2nd block
...
//here we mesh up
   ...
   for(i=0;i<sizeof(dataBlock11);i++)
   {
    dataBlock11[i]='\0';
   }
...
if(pname.length()>16)
   {
    for(i=16;i<pname.length();i++)
    {
      dataBlock11[i-16]=pname[i];
    }
 }  
    //byte trailerBlock   = 7;
    //MFRC522::StatusCode status;
    ...
    Serial.print(F("Writing data into block ")); Serial.print(blockAddr);
    Serial.println(F(" ..."));
    dump_byte_array(dataBlock11, 16); Serial.println();
    status = (MFRC522::StatusCode) mfrc522.MIFARE_Write(blockAddr, dataBlock11, 16);
    if (status != MFRC522::STATUS_OK) {
        Serial.print(F("MIFARE_Write() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
    }
...
//end Sector 1 block 5 data in 2nd block 
  ...
  for(i=0;i<sizeof(dataBlock2);i++)
  {
    dataBlock2[i]='\0';
  }


  //mashing up part 2 ends
    
    for(i=0;i<value.length();i++)
    {
      dataBlock2[i]=value[i];
    }
    ...
    status = (MFRC522::StatusCode) mfrc522.MIFARE_Write(blockAddr2, dataBlock2, 16);
    if (status != MFRC522::STATUS_OK) {
        Serial.print(F("MIFARE_Write() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
    }
